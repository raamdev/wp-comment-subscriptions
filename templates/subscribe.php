...
if (!empty($email)){
	...
	if (get_option('wp_comment_subscriptions_enable_admin_messages', 'no') == 'yes'){
		...
		$subject = __('New subscription to','wp-comment-subscriptions')." $target_post->post_title";
		$message = __('New subscription to','wp-comment-subscriptions')." $target_post->post_title\n".__('User:','wp-comment-subscriptions')." $clean_email";
	...
	if (get_option('wp_comment_subscriptions_enable_double_check', 'no') == 'yes' && !$wp_comment_subscriptions->is_user_subscribed($post_ID, $clean_email, 'C')){
		$wp_comment_subscriptions->add_subscription($post_ID, $clean_email, 'YC');
		$wp_comment_subscriptions->confirmation_email($post_ID, $clean_email);
		$message = html_entity_decode(stripslashes(get_option('wp_comment_subscriptions_subscription_confirmed_dci')), ENT_COMPAT, 'UTF-8');
	}
	else{
...
<p><?php 
$message = str_replace('[post_permalink]', $post_permalink, html_entity_decode(stripslashes(get_option('wp_comment_subscriptions_subscribe_without_commenting')), ENT_COMPAT, 'UTF-8'));
if(function_exists('qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage')){
	$message = str_replace('[post_title]', qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($target_post->post_title), $message);
	$message = qtrans_useCurrentLanguageIfNotFoundUseDefaultLanguage($message);
}
...
<fieldset style="border:0">
	<p><label for="sre"><?php _e('Email','wp-comment-subscriptions') ?></label> <input type="text" class="subscribe-form-field" name="sre" value="<?php echo $email ?>" size="22" onfocus="if(this.value==this.defaultValue)this.value=''" onblur="if(this.value=='')this.value=this.defaultValue"/>
